...
function hexToAscii(hexStr) {
  return hexStr.split(" ").map(el => String.fromCharCode(parseInt(el, 16))).join("");
}
...
// pretty straight-forward, even though it could take some guesses to get there
// hex -> ascii -> b64 decoded
function decodeResponse() {
  return [str1, str2].map(el => b64Decode(hexToAscii(el)));
}
